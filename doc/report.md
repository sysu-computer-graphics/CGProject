...
| 姓名   | 学号     | 分工                                                         | Github                                    | 贡献率 (%) |
| ------ | -------- | ------------------------------------------------------------ | ----------------------------------------- | ---------- |
| 邵梓硕 | 16340189 | 天空盒、贴图映射、阴影映射、简单光照、目标物体、碰撞检测的一部分 | [lossatsea](https://github.com/lossatsea) | 25         |
| 伍正涛 | 16340246 | 项目构建、配置环境、模型寻找、处理、加载，重力系统，碰撞检测 | [wuzht](https://github.com/wuzht)         | 25         |
| 沈大伟 | 16340190 | 模型寻找、场景处理，复杂光照处理                             | [chain](https://github.com/freesdw)       | 25         |
| 李杰泓 | 16340118 | 实现利用鼠标、键盘控制人物移动；实现子弹射击; 显示游戏信息;  | [carolsum](https://github.com/carolsum)   | 25         |
...
负责内容包括：天空盒、贴图映射、阴影映射、简单光照、目标物体类，还有在碰撞检测里障碍物部分稍微参与了一下。

最麻烦的地方应该还是阴影部分，首先关于物体渲染用到的GLSL文件里就包括了光照的内容，因此可以说光照和阴影处理是连在一起的，修改阴影的着色器代码时也要注意不要影响光照的部分。其次，阴影用到的是上课时学到的阴影贴图映射，需要对场景中的物体进行两次渲染：一次采取深度贴图，一次真正渲染物体，由于两次渲染中物体的位置、角度、相对关系等是一样的，因此这部分我学习其他项目用一个类似于renderScene的函数封装起来，调用两次即可。

另外，也是在阴影部分，遇到了显示不出阴影或者是显示不出物体这样的问题，具体可见上面的“遇到的问题和解决方案”。和阴影映射一样，这部分代码都集中在ScenceController类的render中。

最后，关于贴图，实现的时候经常出现贴图混乱的情况，特别是加载外部模型的时候，莫名其妙的就会有一个类型的贴图加载不出来这样的情况出现。之后仔细想了一下应该是我们没有正确地分配贴图编号的问题，比如说不管是加载还是使用，反复地利用 TEXTURE0 和 TEXTURE1 这两个量，导致加载的贴图映射乱七八糟，考虑到这一点，今后的开发中关于贴图资源的统一管理是必不可少的。

在项目展示时没想到和前面一组撞车，而且人家还比我们做的好，功能也比我们实现的多，当时我在台下的心情十分复杂，完全不想上去讲，真是公开处刑......
